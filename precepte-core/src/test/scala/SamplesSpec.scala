...
class SamplesSpec extends FlatSpec with ScalaFutures {
  ...
  class Logger {
    ...
    def info(m: String): Pre[Unit] = println(m).point[Pre]
  }
  ...
  class DB {
    ...
    def findById(id: Long): Pre[Option[Stuff]] = PreMP { s:ST[Logger] =>
      for {
        ...
        r <- Pre.liftF(Some(Stuff(id, "toto")).point[Future])
      } yield (r)
    }
  }
  ...
  object ApiController {
    ...
    def pay(id: Long, amount: Double): DefaultPre[Future, Ctx, Option[PayResult]] = PreMP { (s: ST[Ctx]) =>
      (for {
        ...
        _     <- trans(s.um.logger.info(s"found stuff $stuff").lift[Option].unify(s))
        r     <- trans(s.um.payment.pay(stuff, amount).lift[Option].unify(s))
      } yield (r)).run
    }
  }
