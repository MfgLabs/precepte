  ...
  "Application" should {
    ...
    "work from within a browser" in new WithApplication {
      new WithBrowser(webDriver = WebDriverFactory(HTMLUNIT), app = app) {
        ...
        browser.$("header h1").first.text must equalTo(
          "Play sample application — Computer database")
        browser.$("section h1").first.text must equalTo("574 computers found")

        browser.$("#pagination li.current").first.text must equalTo(
          "Displaying 1 to 10 of 574")

        browser.$("#pagination li.next a").click()

        browser.$("#pagination li.current").first.text must equalTo(
          "Displaying 11 to 20 of 574")
        ...
        browser.$("div.error label").first.text must equalTo(
          "Discontinued date")
        ...
        browser.$("section h1").first.text must equalTo("574 computers found")
        browser.$(".alert-message").first.text must equalTo(
          "Done! Computer Apple II has been updated")
