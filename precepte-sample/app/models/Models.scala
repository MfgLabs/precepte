...
final case class Company(id: Option[Long] = None, name: String)
final case class Computer(id: Option[Long] = None,
                          name: String,
...
/**
  ...
  */
final case class Page[A](items: Seq[A], page: Int, offset: Long, total: Long) {
...
object Models {
  ...
  def Timed[A](f: ST[Unit] => Future[A])(
      implicit callee: Callee): DPre[Future, Unit, A] =
    Precepte(BaseTags(callee, Category.Database)) { (st: ST[Unit]) =>
      f(st)
    }
}
      ...
      Future.successful {
        db.withConnection { implicit connection =>
          SQL("select * from computer where id = {id}")
            .on('id -> id)
            .as(ComputerDB.simple.singleOpt)
        }
      }
    }
  ...
  def list(page: Int = 0,
    ...
    Models.Timed { (st: ST[Unit]) =>
      ...
      logger.debug(s"Listing all computers",
                   Macros.params(page, pageSize, orderBy, filter))
      Future.successful {
    ...
    Future.successful {
      db.withConnection { implicit connection =>
        SQL("select * from company order by name")
          ...
          .flatMap { company =>
            company.id match {
              case Some(l) => List(l.toString -> company.name)
              case _       => Nil
            }
          }
      }
