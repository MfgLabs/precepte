...
trait Monitored[C, A] {
  val f: Context[C] => A
  def apply(c: Context[C]) = f(c)
  def run(c: Context[C]) = apply(c)
}
...
object Monitored {
  ...
  def toT[C, F[_], G[_], A, T[_[_], _]](f: F[G[A]] => T[F, A])(m: Monitored[C, F[G[A]]]): Kleisli[({ type λ[α] = T[F, α] })#λ, Context[C], A] = {
    type Trans[α] = T[F, α]
  ...
  def eitherT[C, F[_], A, B](m: Monitored[C, F[A ∨ B]]): Kleisli[({ type λ[α] = EitherT[F, A, α] })#λ, Context[C], B] = {
  ...
  def apply[C, A](λ: Context[C] => A): Monitored[C, A] =
    new Monitored[C, A] { val f = λ }
