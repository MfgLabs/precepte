...
class PrecepteSpec extends FlatSpec with ScalaFutures with Inside {
      ...
      Precepte(tags("simple.f2")) { (_: ST[Unit]) =>
        Applicative[Future].pure(s"foo $i")
          ...
          Precepte(tags("simple.f2.1")) { (_: ST[Unit]) =>
            Applicative[Future].pure(s"$a.1")
        })
        .flatMap(a =>
          Precepte(tags("simple.f2.2")) { (_: ST[Unit]) =>
            Applicative[Future].pure(s"$a.2")
        })
    def f3(s: String) =
      Precepte(tags("simple.f3")) { (_: ST[Unit]) =>
        Applicative[Future].pure(s"$s finito")
      }
  ...
  it should "OptT" in {
    val f1 = Precepte(tags("opt"))((_: ST[Unit]) =>
      Applicative[Future].pure(Option("foo")))
    val f2 = Precepte(tags("opt"))((_: ST[Unit]) =>
      Applicative[Future].pure(Option(1)))
    val f3 = Precepte(tags("opt"))((_: ST[Unit]) =>
      Applicative[Future].pure((None: Option[Int])))
  ...
  it should "not stack overflow" in {
    def pre(l: List[Int], i: Int) =
      Precepte(tags(s"stack_$i"))((_: ST[Unit]) => Applicative[Future].pure(l))
  ...
  it should "compile using nat" in {
    ...
    val a0 = new CatsExt(res).compile(nat1, nat2).eval(nostate).futureValue
    a0 should ===("6")
  }
