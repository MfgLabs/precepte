  ...
  it should "real world wb.fr home" in {
    ...
    object CardComp {
      ...
      def cardsInfos(cs: List[(Int, Card)], pin: Option[Int]) = Logged(tags("CardComp.cardsInfos")) { (logger: Log) =>
        ...
        List(
          ...
          Card("bar") -> List(Community("community 2"))).pure[Future]
      }
    }
    ...
    object HighlightComp {
      def get() = Logged(tags("HighlightComp.get")) { (logger: Log) =>
        ...
        Highlight("demo", new URL("http://nd04.jxs.cz/641/090/34f0421346_74727174_o2.png")).pure[Future]
      }
    }
    ...
    val getPin =
      (for {
        ...
        id  <- trans(Precepte(tags("pure"))((_: ST[ST]) => b.pin.pure[Future]))
...
/*
  ...
  it should "mapStepState" in {
    import scalaz.~>
    def f1 = Precepte(tags("simple.f1")){(_: ST[Unit]) => 1.pure[Future]}
    def f2(i: Int) = Precepte(tags("simple.f2")){(_: ST[Unit]) => s"foo $i".pure[Future]}
  ...
  it should "iso" in {
    def Precepte(l: List[Int], i: Int) = Precepte(tags(s"stack_$i"))((_: ST[Unit]) => l.pure[Future])
