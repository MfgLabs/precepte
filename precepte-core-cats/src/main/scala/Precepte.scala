...
import cats.{ Monad, Applicative, Functor, Semigroup, ~> }
          ...
          case mfs@MapFusionStep(p0, f0, f1, s0) =>
            ...
            def fusionStep[C, I, D](p: PX[C], f0: C => I, f1: I => D, s: S, depth: Int): F[(S, D)] = {
              p.resume(0)(s) match {
                ...
                case ReturnStep(a, s) => mo.pure(s -> a).map { case (s2, a2) => (s2, f1(f0(a2))) }
...
trait PrecepteBuilder[Ta] {
  ...
  import scala.concurrent.Future
  def future[M, U, A](λ: Precepte[Ta, M, U, Future, A]#S => Future[A])(implicit func: Functor[Future], ec: scala.concurrent.ExecutionContext): Precepte[Ta, M, U, Future, Xor[Throwable, A]] =
    apply { pa =>
